...
// ─────────────────────────────────────────────────────────────
const COLOR_CHAOS = 0xd9b65d; // Azul Celeste (Hero / Inicio)
const COLOR_FORMING = 0x38a3a5; // Turquesa elegante (Al formar la esfera)
const COLOR_SPHERE = 0x57cc99; // Verde Menta fresco (Esfera formada)
const COLOR_EXPLOSION = 0xfaf8f4; // Blanco roto/brillante (Pico de la explosión)
const COLOR_FALLING = 0xd9b65d; // Dorado intenso (Lluvia de partículas)
const COLOR_REFORM = 0xd9b65d; // Nude / Arena suave (Cierre final)
...
function generateExplosionPositions(count, force) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    pos[i * 3 + 1] =
      r * Math.sin(theta) * Math.sin(phi) + (Math.random() - 0.5) * 2;
...
export default function ThreeBackground() {
  ...
  useEffect(() => {
    ...
    geometry.setAttribute(
      "position",
      new THREE.BufferAttribute(chaosPos.slice(), 3),
    );
    ...
    function lerpColor(cA, cB, t) {
      material.color.setRGB(
        (((cA >> 16) & 255) + (((cB >> 16) & 255) - ((cA >> 16) & 255)) * t) /
          255,
    ...
    const animate = () => {
      ...
      particles.rotation.y +=
        (state.mouseX * 0.35 - particles.rotation.y) * 0.012;
      particles.rotation.x +=
        (state.mouseY * 0.22 - particles.rotation.x) * 0.012;
